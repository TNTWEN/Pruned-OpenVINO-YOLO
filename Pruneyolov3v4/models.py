...
def create_modules(module_defs, img_size, cfg):
    ...
    img_size = [img_size] * 2 if isinstance(img_size, int) else img_size  # expand if necessary  如果是416->[416,416
    ...
    for i, mdef in enumerate(module_defs):
        modules = nn.Sequential()#给CBL这类可以直接包装成Sequential(),免去自定义forward的层使用

        if mdef['type'] == 'convolutional':
            bn = mdef['batch_normalize'] #int
            #+++++++++++++++++++++++++ insert +++++++++++++++++++++++++#
            # filters = mdef['filters']#本来就是int 不改应该没事
            filters = int(mdef['filters'])#输出通道，最后加入到output_filters中
            #+++++++++++++++++++++++++ insert end++++++++++++++++++++++#
            ...
            if bn:
                ...
                routs.append(i)  # detection output (goes into yolo layer) yolo层前的卷积无bn
            ...
            modules = WeightedFeatureFusion(layers=layers, weight='weights_type' in mdef) #weight 挖坑
            ...
            modules = YOLOLayer(anchors=mdef['anchors'][mdef['mask']],  # anchor list 对于np.array, array[[3,4,5]]起到mask作用
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        if isinstance(cfg, str):
            self.module_defs = parse_model_cfg(cfg)
            cfg_path = cfg
        elif isinstance(cfg, list):#slim_prune
